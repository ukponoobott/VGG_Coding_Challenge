...
def auth():
    ...
    user = dict([("user_id", account[0]), ("username", account[1]), ("password", account[2])])
...
def projects():
    if "username" in session:
        if request.method == "GET":
            ...
            with sqlite3.connect("database.db") as con:
                cur = con.cursor()
                cur.execute('INSERT INTO projects(project_name, description, completed) VALUES (?, ?, ?)',
                            (project_name, description, completed))
                con.commit()
...
def project_actions_update(project_id, actions_id):
    if "username" in session:
        if request.method == "GET":
            con = sqlite3.connect("database.db")
            cur = con.cursor()
            cur.execute("SELECT * FROM actions where project_id = {} AND actions_id = {}".format(project_id,
                        actions_id))
            rows = cur.fetchone()
            return jsonify(rows), 200
